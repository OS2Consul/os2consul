...
class Users::SessionsController < Devise::SessionsController
  ...
  def new
    if Rails.application.secrets.saml_ip_ranges.present? && Rails.application.secrets.saml_ip_ranges.split(',').map(&:strip).any? { |ip_range| IPAddr.new(ip_range).include? request.remote_ip }
